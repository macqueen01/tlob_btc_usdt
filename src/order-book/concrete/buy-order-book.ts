...
export class BuyOrderBookChange extends OrderBookChange {
    get On(): "BUY" {
        return "BUY"
    };
};

export class BuyOrderBook implements OrderBook {
    ...
    apply(change: BuyOrderBookChange): void {
        if (change.On != "BUY") throw new Error("Applying Wrong Change to BuyOrderBook");
        ...
        if (change.Type == "SET") {
            ...
            if (index === -1 && change.Size !== 0) {
                ...
                this.sortedPrices.sort((a, b) => b - a);  // Descending for buy
